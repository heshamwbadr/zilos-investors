  ...
  return (
    <main className="bg-slate-950 text-slate-200 w-full h-screen overflow-y-scroll snap-y snap-mandatory scroll-smooth no-scrollbar">
      ...
      <SectionWrapper id="business-model" className={pricingUnlocked ? 'pb-32' : ''}>
                  ...
                  <table className="w-full text-left text-sm border-collapse min-w-[600px]">
                    <thead>
                      <tr className="border-b border-slate-800">
                        <th className="py-4 px-4 font-medium text-slate-500 w-1/4"></th>
                        <th className="py-4 px-4 font-semibold text-white w-1/4">Essentials</th>
                        <th className="py-4 px-4 font-semibold text-white w-1/4">Growth</th>
                        <th className="py-4 px-4 font-semibold text-white w-1/4">Platinum — Partnership</th>
                      </tr>
                    </thead>
                    <tbody>
                      {COMPARISON_TABLE.map((row, idx) => (
                        <tr key={idx} className="border-b border-slate-800/50 last:border-0 hover:bg-slate-800/30 transition-colors">
                          <td className="py-4 px-4 font-medium text-slate-400">{row.feature}</td>
                          <td className="py-4 px-4 text-slate-300">{row.essentials}</td>
                          <td className="py-4 px-4 text-slate-300">{row.growth}</td>
                          <td className="py-4 px-4 text-slate-300">{row.platinum}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <div className="mt-12 p-4 bg-emerald-900/10 border border-emerald-900/30 rounded text-center max-w-2xl relative group">
